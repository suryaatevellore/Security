...
def service_connection_client(key, mask, password):
    try:
        ...
        if mask & selectors.EVENT_READ:
            # print("Entered client for reading")
            recv_data_client = sock.recv(64000)  # Should be ready to read
            if recv_data_client:
                # print("Should be entering this first")
                data_client.data_client_rcvd+=recv_data_client
                # print("Recvd data ",data_client.data_client_rcvd)


            if not recv_data_client and len(data_client.data_client_rcvd)>16:
                # print("Recieved data", data_client.data_client_rcvd)
                perform_decryption(data_client.data_client_rcvd, password)
                sel.unregister(sock)
                sock.close()

            if data_client.data_client_rcvd==b'bye':
                sock.shutdown(socket.SHUT_RD)


        if mask & selectors.EVENT_WRITE:
            # if not data_client.messages:
            #     # print("Bye for writing", len(b'bye'))
            #     sock.send(b"bye")
...
def service_connection(key, mask, password):
    # print("Looking to service the server")
    try:
        sock = key.fileobj
        data = key.data
        if mask & selectors.EVENT_READ:
            recv_data = sock.recv(64000)
            sock.settimeout(5.0)  # Should be ready to read
            if recv_data:
                # print(f"Receieved Data {recv_data} , length {len(recv_data)}")
                data.inb += recv_data
